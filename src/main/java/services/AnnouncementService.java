...
public class AnnouncementService implements IService<Announcement> {
    ...
    public List<Announcement> findByFilters(String keyword, LocalDate date, Announcement.Zone zone, int driverId) throws SQLException {
        List<Announcement> announcements = new ArrayList<>();
        StringBuilder sql = new StringBuilder("SELECT * FROM announcement WHERE id_transporter = ?");

        List<Object> parameters = new ArrayList<>();
        parameters.add(driverId); // Ajouter l'ID du conducteur connecté
